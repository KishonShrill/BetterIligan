...
export async function postBoardMessage(
    ...
    try {
        ...
        if (!rateLimit(ip, 3, 60_000)) {
            return { success: false, error: 'You are posting too fast. Please wait a minute.' };
        }
...
export async function submitHazardReport(
    ...
    try {
        ...
        const { incidentType, barangay, landmark, description, contactNumber } = parsed.data;
        const db = await getDb();
        await db
            .prepare(
                'INSERT INTO bangon_incidents (id, incident_type, barangay, landmark, description, contact_number) VALUES (?1, ?2, ?3, ?4, ?5, ?6)',
            )
            .bind(crypto.randomUUID(), incidentType, barangay, landmark ?? null, description, contactNumber)
            .run();
