...
async function setBoardStatus(
  ...
  await db
    .prepare(
      "UPDATE bangon_board_messages SET status = ?1, updated_at = datetime('now') WHERE id = ?2",
    )
    .bind(status, id)
    .run();
...
export async function unpublishBoardMessage(id: string): Promise<void> {
  ...
  await db
    .prepare(
      "UPDATE bangon_board_messages SET status = 'pending', updated_at = datetime('now') WHERE id = ?1",
    )
    .bind(id)
    .run();
...
export async function deleteBoardMessage(id: string): Promise<void> {
  ...
  const db = await getDb();
  await db
    .prepare("DELETE FROM bangon_board_messages WHERE id = ?1")
    .bind(id)
    .run();
...
async function setIncident(
  ...
  await db
    .prepare(
      "UPDATE bangon_incidents SET verified = ?1, status = ?2, updated_at = datetime('now') WHERE id = ?3",
    )
    .bind(verified, status, id)
    .run();
  await audit(
    verified ? "incident:verified" : "incident:dismissed",
    "incident",
    id,
  );
  revalidatePath("/bangon-iligan");
...
export async function unverifyIncident(id: string): Promise<void> {
  ...
  await db
    .prepare(
      "UPDATE bangon_incidents SET verified = 0, status = 'reviewing', updated_at = datetime('now') WHERE id = ?1",
    )
    .bind(id)
    .run();
...
export async function resolveIncident(id: string): Promise<void> {
  ...
  await db
    .prepare(
      "UPDATE bangon_incidents SET status = 'resolved', updated_at = datetime('now') WHERE id = ?1",
    )
    .bind(id)
    .run();
...
export async function activateIncident(
  ...
  if (!title)
    return {
      success: false,
      error: "A title is required to declare an incident.",
    };
...
export async function deactivateIncident(): Promise<ActionResult> {
  ...
  try {
    ...
    await db
      .prepare(
        "UPDATE bangon_incident_state SET active = 0, updated_at = datetime('now') WHERE id = 'current'",
      )
      .run();
  } catch (err) {
