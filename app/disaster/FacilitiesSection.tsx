...
export default function FacilitiesSection() {
    ...
    const pinColors: Category[] = PIN_POSITIONS.map((_, i) => present[i % present.length]);

    return (
        <section aria-labelledby="facilities-heading">
            <h2 id="facilities-heading" className="text-2xl font-bold text-slate-900 mb-2">
                Emergency Facilities Map
            </h2>
            <p className="text-slate-600 mb-5">
                Find the nearest hospital, fire or police station, utility office,
                or evacuation landmark across Iligan City.
            </p>
            ...
            <div className="rounded-lg border border-gray-200 bg-white shadow-sm overflow-hidden">
                ...
                >
                    {PIN_POSITIONS.map((pos, i) => (
                        <MapPin
                            key={i}
                            className="absolute w-7 h-7 -translate-x-1/2 -translate-y-full drop-shadow"
                            style={{ top: pos.top, left: pos.left, color: CATEGORY_META[pinColors[i]].color, fill: "#fff" }}
                        />
                    ))}
                ...
                <div className="p-6 flex flex-col gap-4 sm:flex-row sm:items-center">
                    <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-x-3 gap-y-1.5 mt-3">
                            {present.map((c) => (
                                <span key={c} className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-600">
