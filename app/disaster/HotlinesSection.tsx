...
> = {
  government: {
    ...
    label: "Rescue & Disaster Response",
    color: "#059669",
    Icon: LifeBuoy,
  },
  ...
  return (
    <section aria-labelledby="hotlines-heading">
      ...
      <p className="mb-6 text-slate-600">
        Tap any number to call. Short codes like 811 or 160 work inside Iligan;
        from outside the city, use the full landline or mobile numbers.
      </p>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {grouped.map(({ category, entries }) => {
          ...
          return (
            ...
            >
              <h3 className="mb-4 flex items-center gap-2.5 text-sm font-bold tracking-wider text-slate-900 uppercase">
                <span
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {hotline.numbers.map((number) => (
                        <a
                          key={number}
                          href={telHref(number)}
                          className="inline-flex items-center gap-1.5 rounded-lg bg-blue-50 px-4 py-2.5 text-base font-bold text-blue-700 transition-colors hover:bg-blue-600 hover:text-white"
                        >
      ...
      >
        ...
        <span className="flex-1">
          ...
          <span className="block text-sm text-slate-500">
            A complete, regularly-maintained list for Iligan City, curated by
            BetterGov.ph.
          </span>
        </span>
        <ArrowRight
          className="h-5 w-5 shrink-0 text-slate-400 transition-all group-hover:translate-x-0.5 group-hover:text-blue-600"
          aria-hidden
        />
      </a>
    </section>
