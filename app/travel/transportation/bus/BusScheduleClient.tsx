...
import { MapPin, ArrowRightLeft, Clock, Info } from 'lucide-react';
...
export default function BusScheduleClient() {
    ...
    // Track current time in minutes from midnight for easy comparison
    ...
    useEffect(() => {
        ...
        // Update every 60 seconds so the UI grays out buses in real-time
        const interval = setInterval(updateTime, 60000);
        return () => clearInterval(interval);
    }, []);

    // --- HELPER: Toggle direction ---
    const toggleDirection = () => {
        setDirection(prev => prev === 'forward' ? 'reverse' : 'forward');
    };

    // --- HELPER: Check if bus departed ---
    const hasDeparted = (timeStr: string) => {
        // If time hasn't loaded yet on the client, assume it hasn't departed
        if (currentMinutes === -1) return false;

        // Parse "4:30 AM" into total minutes from midnight
        ...
        // Returns true if the current time has passed the trip time
    ...
    return (
        <div className="max-w-250 mx-auto px-4 md:px-6 py-8 space-y-8">

            {/* --- COMPANY TABS --- */}
            <div className="flex p-1 bg-slate-200/60 rounded-xl w-fit mx-auto">
