...
const DisasterMapLeaflet = dynamic(
  () => import("@/app/disaster/map/DisasterMapLeaflet"),
  {
    ssr: false,
    loading: () => (
...
export default function BangonCommandCenter({
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem("bangon-standby-dismissed"))
    ...
    <div className="relative h-[100dvh] w-full overflow-hidden bg-slate-100">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 z-[700] flex items-start justify-between gap-3 p-3 sm:p-4">
        ...
        <div className="pointer-events-auto flex items-center gap-2 rounded-xl bg-white/95 px-2.5 py-2 shadow-md backdrop-blur sm:gap-3 sm:px-3">
          ...
          <span className="text-sm font-bold tracking-tight whitespace-nowrap text-slate-900">
            Bangon Iligan
          </span>
      ...
      {!active && !standbyDismissed && (
        <div className="pointer-events-none absolute inset-x-0 top-16 z-[550] flex justify-center px-3 sm:top-20">
          <div className="pointer-events-auto flex max-w-2xl items-start gap-2 rounded-xl bg-emerald-900/85 py-2 pr-2 pl-4 text-xs text-emerald-50 shadow-lg backdrop-blur sm:text-sm">
      ...
      {!panelOpen && (
        <button
          ...
          className="pointer-events-auto absolute right-3 bottom-4 z-[600] inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-3 text-sm font-bold text-white shadow-lg transition-colors hover:bg-slate-800 sm:right-4"
        >
      ...
      <div
        className={`pointer-events-none absolute top-0 right-0 z-[650] flex h-[100dvh] w-full flex-col p-3 transition-transform duration-300 sm:w-[380px] sm:p-4 ${
...
function StatusPill({ active, label }: { active: boolean; label: string }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-bold tracking-wider uppercase ${
...
function FacilityCard({
  ...
  return (
    <div className="pointer-events-auto absolute bottom-4 left-1/2 z-[700] w-[calc(100%-1.5rem)] max-w-sm -translate-x-1/2 rounded-2xl bg-white p-4 shadow-2xl sm:left-4 sm:translate-x-0">
      ...
      <h3 className="mt-1.5 pr-6 text-base leading-tight font-bold text-slate-900">
        {facility.name}
      </h3>
      {facility.address && (
