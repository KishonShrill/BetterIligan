...
export default async function BangonIliganPage() {
  ...
  const [config, admin] = await Promise.all([
    getEffectiveBangonConfig(),
    isAdmin(),
  ]);
  const [messages, reports, pendingMessages, pendingReports, feed] =
    await Promise.all([
      config.boardEnabled ? getApprovedBoardMessages() : Promise.resolve([]),
      getVerifiedIncidents(50, !admin),
      admin && config.boardEnabled
        ? getPendingBoardMessages()
        : Promise.resolve([]),
  ...
  const mapFacilities = disasterFacilities.filter(
    (f) => f.category !== "barangay",
  );
