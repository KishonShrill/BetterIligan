...
export async function getApprovedBoardMessages(
  limit = 50,
...
export async function getVerifiedIncidents(
  limit = 50,
  mask = true,
    ...
    return (results ?? []).flatMap((row) => {
      ...
      return [
        mask
          ? {
              ...parsed.data,
              contact_number: maskPhone(parsed.data.contact_number),
            }
          : parsed.data,
      ];
    });
...
export async function getPendingBoardMessages(
  limit = 100,
): Promise<BoardMessageRow[]> {
  try {
    const db = await getDb();
...
export async function getUnverifiedIncidents(
  limit = 100,
): Promise<IncidentReportRow[]> {
  try {
    const db = await getDb();
...
export async function getIncidentState(): Promise<IncidentStateRow | null> {
  try {
    ...
    const { results } = await db
      .prepare(
        "SELECT * FROM bangon_incident_state WHERE id = 'current' LIMIT 1",
      )
      .all();
    const row = (results ?? [])[0];
    if (!row) return null;
