...
// 1. Define shared structural types for the component to expect
...
export interface BaseCodeEntryData {
  image?: string;
  places?: string[];
  routeFare?: BaseFare;
  description?: string;
...
// 2. Define the component props using our base types
interface RouteDetailsProps {
  route: BaseRouteData | null;
  codeEntry?: BaseCodeEntryData;
...
export default function RouteDetails({
  ...
  // Auto-expand whenever a new route is selected
  useEffect(() => {
    if (route) {
  ...
  return (
    <aside className="animate-in slide-in-from-right-8 absolute top-4 right-4 z-[1000] flex max-h-[calc(100dvh-5.75rem)] w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl transition-all duration-300 sm:w-80">
      {isMinimized ? (
        /* MINIMIZED VIEW */
        <div className="flex items-center justify-between bg-white p-3">
          <div className="flex min-w-0 items-center gap-3">
            <div
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-[10px] font-extrabold text-white shadow-sm"
          ...
          <div className="relative shrink-0">
            ...
            {image ? (
              <div className="relative h-44 w-full">
                <Image
                  src={image}
                  alt={
                    route?.routeName ? route.routeName : (route.name as string)
                  }
                  fill
            ...
            <div className="absolute top-3 right-3 flex gap-2">
              <button
                ...
                aria-label="Minimize route details"
